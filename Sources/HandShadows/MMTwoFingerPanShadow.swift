...
public class MMTwoFingerPanShadow: NSObject {
    var isRight: Bool
    ...
    var openMiddleFingerTipPath: UIBezierPath
    var openIndexFingerTipPath: UIBezierPath
    var closedMiddleFingerTipPath: UIBezierPath
    ...
    init(forRightHand _isRight: Bool) {
        ...
        if isRight {
            ...
            flipPathAroundYAxis(openMiddleFingerTipPath)
            flipPathAroundYAxis(openIndexFingerTipPath)
            flipPathAroundYAxis(closedMiddleFingerTipPath)
            flipPathAroundYAxis(closedIndexFingerTipPath)
        }
    }

    func avgPoint(p1: CGPoint, withPoint p2: CGPoint, weight: CGFloat) -> CGPoint {
        return CGPoint(x: p1.x*weight + p2.x*(1-weight), y: p1.y*weight + p2.y*(1-weight))
    }
    ...
    public func openTo(openPercent: CGFloat) {
        ...
        for i in 0..<openPath.elementCount {
            ...
            case .moveToPoint:
                lastInterpolatedPath.move(to: avgPoint(p1: openElement.points[0], withPoint: closedElement.points[0], weight: openPercent))
            case .addLineToPoint:
                lastInterpolatedPath.addLine(to: avgPoint(p1: openElement.points[0], withPoint: closedElement.points[0], weight: openPercent))
            case .addQuadCurveToPoint:
                let endPt = avgPoint(p1: openElement.points[1], withPoint: closedElement.points[1], weight: openPercent)
                let ctrlPt = avgPoint(p1: openElement.points[0], withPoint: closedElement.points[0], weight: openPercent)
                lastInterpolatedPath.addQuadCurve(to: endPt, controlPoint: ctrlPt)
            case .addCurveToPoint:
                let endPt = avgPoint(p1: openElement.points[2], withPoint: closedElement.points[2], weight: openPercent)
                let ctrlPt1 = avgPoint(p1: openElement.points[0], withPoint: closedElement.points[0], weight: openPercent)
                let ctrlPt2 = avgPoint(p1: openElement.points[1], withPoint: closedElement.points[1], weight: openPercent)
